...
<cli_script> = "<module_name>.cli.app:main"
...
"aiosqlite>=0.22",
"pytest-asyncio>=1.0",  # required — ferro is async
...
## observability.py (structlog + optional logfire)
...
def _send_to_logfire() -> bool | str:
    raw = os.environ.get("LOGFIRE_SEND_TO_LOGFIRE", "if-token-present")
    return False if raw.lower() in ("false", "0", "no", "off") else raw
...
def configure_observability(*, service_name: str, verbose: bool = False) -> None:
  ...
  # Optional: import logfire only when telemetry is enabled at scaffold time
  # import logfire
  # logfire.configure(service_name=service_name, send_to_logfire=_send_to_logfire(), console=False)
...
When Logfire is enabled, expand to the full facade pattern: configure logfire first, structlog second, then re-export `span = logfire.span` so callers never import `logfire` or `structlog` directly.
...
`pyproject.toml` entry: `<cli_script> = "<module_name>.cli.app:app"` or wrap with a `main()` that calls `app()`.
...
Set `asyncio_mode = "auto"` in `[tool.pytest.ini_options]` when using ferro.
