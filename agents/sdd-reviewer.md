...
1. Otherwise run `/review-pr <PR#>` and follow it exactly: gather, run **Verify** yourself in a clean checkout, judge **spec** against the sub-issue body, judge **quality** against the repo's laws, post one structured review comment with `gh pr review --comment`, update the progress comment.
