---
name: sync-progress
description: Upsert an idempotent progress comment on a GitHub issue under a marker, and claim or unclaim the issue by assignment. Use when a worker or orchestrator needs to record status on a sub-issue or epic without spamming comments.
...
| A blocker was discovered that the tracker does not know about | `blocked` + `gh issue edit N --add-blocked-by <M>` | — |
