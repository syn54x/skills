...
- The PR's base is the repo's default branch (or the branch the user names as the release line). If the base is an integration branch, stop and run `/review-pr` instead; a panel on a slice is noise at nine times the cost.
...
From [references/personas.md](references/personas.md). Selection is judgment about what the diff touches, not keyword matching; count only executable code toward line thresholds.
