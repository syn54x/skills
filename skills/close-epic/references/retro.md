...
Everything below is `gh` + `jq`. `scripts/sdd-retro.sh` in the plugin does the same thing in one call.
...
| `escalations.needs_info` | timeline `labeled` events for `needs-info` on sub-issues | ticket quality (`to-tickets-plus`) |
...
| `edges_added` | `Edges added:` line in the `<!-- sdd-wave -->` comment on the epic | safety-check misses (`to-tickets-plus`, `build-epic` §3) |
...
| `verify_corrections` | PR bodies containing `Verify block corrected: yes` | Verify blocks that lied (`to-tickets-plus`) |
...
|---|---|
| `verify_corrections` high | `to-tickets-plus` §3: Verify blocks written without running them |
| `escalations.needs_info` high | `to-tickets-plus` placeholder gate, or `to-spec` leaving decisions open |
| `edges_added` high | `to-tickets-plus` §5 / `parallel-safety-check` missed interface pairs |
