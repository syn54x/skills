...
How a **[skill]** learning becomes an issue on `syn54x/skills`, and what it may contain. The rules exist so that nothing about the user's code, repo or organisation leaves their GitHub without their explicit, per-issue approval.
...
`scripts/feedback-guard.sh <body-file>` in the plugin runs these checks and exits 2 on a hit, naming the rule. Without the plugin, run them by eye against the list above before every filing.

## Filing

```bash
gh issue create -R syn54x/skills --label skill-feedback \
  --title "to-tickets-plus §3: verify-block-wrong (2/6 tickets)" \
  --body-file /tmp/feedback-1.md
# or, to file from the browser:
gh issue create -R syn54x/skills --label skill-feedback --title "…" --body-file /tmp/feedback-1.md --web
```
